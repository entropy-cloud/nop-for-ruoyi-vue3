...
import { clearAuthToken, getAuthToken, getTenantId, setAuthToken } from './auth';
...
ajax.interceptors.response.use(
	res => {
		const token = res.headers["nop-token"]
		if (token) {
			setAuthToken(token)
		}
		return res
...
export function ajaxFetch(options: FetcherRequest): Promise<FetcherResult> {
	...
	const apiUrl = useGlobalSetting().apiUrl
	if (apiUrl && config.url?.startsWith("/")) {
...
function prepareHeaders(config: AxiosRequestConfig, opts: ExtOptions) {
	...
	if (token && opts.withToken !== false) {
		...
		config.headers["nop-tenant"] = tenantid;
		//--update-begin--author:liusq---date:20220325---for: 增加vue3标记
		config.headers["nop-version"] = globalVersion;
		...
		// 	config.headers["x-low-app-id"] = routeParams.appId as string;
		// 	// lowApp自定义筛选条件
